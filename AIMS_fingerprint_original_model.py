...
class random_crop_tensor(torch.nn.Module):
    def __init__(self, scale, scale_2=1, img_size=448):
        self.scale = scale
    ...
    def forward(self, img):
        ...
        img_num = 0
        while True:
            #img_crop = transforms.RandomResizedCrop(448, scale = (self.scale, self.scale), antialias=True)(img)
            output_size = (int(w/self.scale), int(h/self.scale))
            
            img_resize = F.interpolate(img.unsqueeze(0), size=output_size, mode='bilinear', align_corners=False).squeeze(0)
            ...
            if img_resize_w < self.img_size:
                img_w_pad = int((self.img_size - img_resize_w) / 2)+1

            if img_resize_h < self.img_size:
                img_h_pad = int((self.img_size - img_resize_h) / 2)+1
            ...
            img_crop = transforms.RandomCrop(self.img_size)(img_resize)

            if self.scale_2 != 1:
                img_crop = transforms.RandomCrop(int(self.img_size/self.scale_2))(img_crop)
                img_crop = transforms.Resize(self.img_size)(img_crop)
...
class random_crop_tensor_test(torch.nn.Module):
    def __init__(self, scale, test_samples, scale_2=1, img_size=448):
        self.scale = scale
    ...
    def forward(self, img):     
        ...
        x = torch.zeros(self.test_samples, 3, self.img_size, self.img_size)
...
class CLAHE(torch.nn.Module):
    """Apply Contrast Limited Adaptive Histogram Equalization to tensor images."""
...
class RandomExposureSimulator(torch.nn.Module):
    """Randomly adjusts brightness, contrast, and gamma to simulate different exposures"""
...
class RandomNoise(torch.nn.Module):
    """Adds different types of noise to simulate image capture variations"""
    ...
    def __init__(self, image_folder_dataset, indices, pre_transform, device, transform=None, rank=None):
        ...
        for i, idx in enumerate(tqdm(indices, desc=f"[Rank {rank}] Loading data", disable=rank is not None and rank != 0)):
            ...
            if not isinstance(img, torch.Tensor):
                img = self.pre_transform(img)  # Apply the transformation
            img = img.to(device).half()  # Convert to half-precision and move to GPU
            self.data.append(img)
            self.targets.append(torch.tensor(label, device=device))
            
            # Log memory usage every 100 images
...
def run_model(local_rank, world_size, rank):
    ...
    try:
        # else:
        lr = 0.0005  # Default value
        model_name = ''  # Default value
        weight_decay = 0.01  # Default value
        lr_gamma = 0.99  # Default value
        test_samples = 8  # Default value
        sigma = 0.01  # Default value
        ...
        sensor_noise = 0.0
        contrast_mod = 0.0  # Default value for contrast_mod
        scale_mod = 0.1  # Default value for scale_mod
        clahe_grid = 64  # Default value for CLAHE grid size
        num_epochs = 50  # Default value for num_epochs
        ...
        if rank == 0:
            ...
            contrast_mod = wandb.config.contrast_mod  # Get contrast_mod from wandb config
            scale_mod = wandb.config.scale_mod  # Get scale_mod from wandb config
            clahe_grid = wandb.config.clahe_grid
            num_epochs = wandb.config.num_epochs
            

        max_length = 50  # Define a maximum length for the model name
        model_name = model_name.ljust(max_length)

        # Broadcast the configuration values from rank 0 to all other ranks
        lr = torch.tensor(lr).cuda()
        model_name = torch.tensor([ord(c) for c in model_name]).cuda()  # Convert string to tensor
        ...
        train_sampler = DistributedSampler(image_datasets['train'], shuffle=True, num_replicas=world_size * 4, rank=rank, drop_last=True)
        ...
        train_transform = transforms.Compose([
                ...
                transforms.RandomAffine(degrees=0, translate=(0.1, 0.1), scale=(0.5, 1.5)),  # More subtle
                ...
                CLAHE(clip_limit=3.0, tile_grid_size=(clahe_grid, clahe_grid)),
                random_crop_tensor(scale=1, scale_2=1),
                PerImageNormalize()])
        
        test_transforms = transforms.Compose([
                random_crop_tensor_test(scale=1, test_samples=test_samples, scale_2=1),
                PerImageNormalize(),
                ])
        
        data_loading_device = torch.device(f'cuda:{local_rank}')
        in_memory_train_dataset = SubsetInMemoryDataset(image_datasets['train'], train_indices, data_transforms['train'], data_loading_device, train_transform, rank=rank)
        in_memory_val_dataset = SubsetInMemoryDataset(image_datasets['val'], val_indices, data_transforms['val'], data_loading_device, test_transforms, rank=rank) 

        dataloaders_dict = {
            'train': DataLoader(in_memory_train_dataset, batch_size=batch_size, shuffle=True, num_workers=num_workers, pin_memory=False),
            'val': DataLoader(in_memory_val_dataset, batch_size=int(batch_size/test_samples), shuffle=False, num_workers=num_workers, pin_memory=False)
        }
        ...
        # Sequential doesn't have an append method
        ...
        # Train the model
        model, acc, trainloss, pred_ids, ground_truth_ids, best_val_loss, best_val_acc = ml_models.train_model(model, model_name, dataloaders_dict, image_datasets, criterion, optimizer_ft, batch_size, class_names, main_dir, test_samples, img_size, local_rank, rank, scheduler, n_train_samples, n_val_samples, val_sampler, num_epochs=num_epochs, jigsaw=jigsaw, train_sampler=None)
        # Print results
        if rank == 0:
...
def run_agent(local_rank, world_size, rank):
    wandb.agent(sweep_id="AIMS/qsds6gvq", count=1, function=lambda: run_model(local_rank, world_size, rank))
