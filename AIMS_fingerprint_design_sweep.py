...
import region_fingerprint_dpp as ml_models
...
class PadToSize:
    ...
    def __call__(self, img):
        ...
        padding = [
            (self.size[1] - img.size()[2]) // 2,  # left
            (self.size[0] - img.size()[1]) // 2,  # top
            (self.size[1] - img.size()[2] + 1) // 2,  # right
            (self.size[0] - img.size()[1] + 1) // 2   # bottom
        ]
...
class SubsetInMemoryDataset(Dataset):
    def __init__(self, image_folder_dataset, indices, pre_transform, device, transform=None, rank=None):
        ...
        total_memory_before = torch.cuda.memory_allocated(device) / (1024 ** 3)  # GB
        ...
        end_time = time.time()
        total_memory_after = torch.cuda.memory_allocated(device) / (1024 ** 2)  # MB
...
def print_subfolder_counts(dataset, dataset_name):
    ...
    for idx in dataset.indices:
        ...
        try:
            ...
            subfolder = parts[class_idx + 1] if class_idx + 1 < len(parts) else "NO_SUBFOLDER"
        except ValueError:
            subfolder = "UNKNOWN"
        if class_name not in counts:
            counts[class_name] = {}
        if subfolder not in counts[class_name]:
            counts[class_name][subfolder] = 0
        counts[class_name][subfolder] += 1
    
    for class_name in counts:
        print(f"  {class_name}:")
        for subfolder in counts[class_name]:
            print(f"    {subfolder}: {counts[class_name][subfolder]} images")
...
def run_model(local_rank, world_size, rank):
    ...
    # set_seed(42 + rank)  # Set a fixed seed for reproducibility
    ...
    try:
        ...
        train_design = 'Connector'
        training_parts_num = 1
        num_train_designs = 1
        ...
        if rank == 0:
            ...
            train_design = wandb.config.train_design
            training_parts_num = wandb.config.training_parts_num
            clahe_grid = wandb.config.clahe_grid
            num_train_designs = wandb.config.num_train_designs

        max_length = 50
        model_name = model_name.ljust(max_length)
        train_design = train_design.ljust(max_length)
        ...
        train_design = torch.tensor([ord(c) for c in train_design]).cuda()
        training_parts_num = torch.tensor(training_parts_num).cuda()
        clahe_grid = torch.tensor(clahe_grid).cuda()
        num_train_designs = torch.tensor(num_train_designs).cuda()
        ...
        dist.broadcast(train_design, src=0)
        dist.broadcast(training_parts_num, src=0)
        dist.broadcast(clahe_grid, src=0)
        dist.broadcast(num_train_designs, src=0)
        ...
        train_design = ''.join([chr(int(c)) for c in train_design.cpu().numpy()]).strip()
        ...
        training_parts_num = training_parts_num.item()
        clahe_grid = clahe_grid.item()
        num_train_designs = num_train_designs.item()
        ...
        c = 'aim_all_designs_all_views_design_separation_20_train_10_val'
        data_dir = 'data/'+ str(c)
        if rank == 0:
        ...
        full_dataset = datasets.ImageFolder(os.path.join(ssd_dir, 'train'), data_transforms['train'])
        
        full_design_list = ['Connector', 'Switch', 'King', 'Target', 'Oring', 'Clip', 'Fishbait', 'Plug', 'Minion']

        train_designs = random.sample(full_design_list, num_train_designs)
        test_designs = [d for d in full_design_list if d not in train_designs]
        
        random.shuffle(test_designs)
        test_designs = test_designs[:4]

        desired_train_subfolders = {
            'Stratasys450mc-1': [str(d) for d in train_designs],  # Convert to strings
            'Stratasys450mc-2': [str(d) for d in train_designs],
            'Stratasys450mc-3': [str(d) for d in train_designs],
            'Stratasys450mc-4': [str(d) for d in train_designs],
            'Stratasys450mc-5': [str(d) for d in train_designs],
            'Stratasys450mc-6': [str(d) for d in train_designs],
        }
        
        # Track unique serial numbers for each class (across ALL subfolders)
        serial_numbers_by_class = {cls: set() for cls in full_dataset.classes}

        # Map from (class, serial_number) to indices across ALL subfolders
        serial_to_indices = {cls: {} for cls in full_dataset.classes}

        # Track which subfolders each serial number appears in
        serial_to_subfolders = {cls: {} for cls in full_dataset.classes}

        # Extract serial numbers from ALL filenames without subfolder filtering
        for idx, (img_path, label) in enumerate(full_dataset.samples):
            class_name = full_dataset.classes[label]
            filename = os.path.basename(img_path)
            serial_number = filename[:6]
            
            serial_numbers_by_class[class_name].add(serial_number)
            
            if serial_number not in serial_to_indices[class_name]:
                serial_to_indices[class_name][serial_number] = []
            serial_to_indices[class_name][serial_number].append(idx)
            
            parts = img_path.replace("\\", "/").split('/')
            subfolder = parts[-2]
            if serial_number not in serial_to_subfolders[class_name]:
                serial_to_subfolders[class_name][serial_number] = set()
            serial_to_subfolders[class_name][serial_number].add(subfolder)

        max_serials_per_class = training_parts_num
        limited_indices = []

        for cls in serial_numbers_by_class:
            training_serials = [serial for serial in serial_numbers_by_class[cls] 
                            if any(sub in serial_to_subfolders[cls].get(serial, set()) 
                                    for sub in desired_train_subfolders[cls])]
            
            if rank == 0:
                print(f"{cls}: Found {len(training_serials)} unique serial numbers in training subfolders")
            
            if len(training_serials) > max_serials_per_class:
                selected_serials = random.sample(training_serials, max_serials_per_class)
            else:
                selected_serials = training_serials
            
            for serial in selected_serials:
                limited_indices.extend(serial_to_indices[cls][serial])
                
        custom_train_dataset = Subset(full_dataset, limited_indices)
        custom_train_dataset.classes = full_dataset.classes
        custom_train_dataset.class_to_idx = full_dataset.class_to_idx
        custom_train_dataset.samples = [full_dataset.samples[i] for i in limited_indices]
        custom_train_dataset.imgs = custom_train_dataset.samples
        
        if rank == 0:
            print("\nSummary of selected samples by serial number:")
            for cls in serial_numbers_by_class:
                selected_serials = set()
                for serial in serial_to_indices[cls]:
                    if any(idx in limited_indices for idx in serial_to_indices[cls][serial]):
                        selected_serials.add(serial)
                
                selected_count = sum(1 for idx in limited_indices 
                                    if full_dataset.samples[idx][1] == full_dataset.class_to_idx[cls])
                
                print(f"\n{cls}: {selected_count} images from {len(selected_serials)} serial numbers")
                
        print_subfolder_counts(custom_train_dataset, "TRAIN")
        
        full_dataset = datasets.ImageFolder(os.path.join(ssd_dir, 'val'), data_transforms['val'])
        
        desired_test_subfolders = {
            'Stratasys450mc-1': test_designs,
            'Stratasys450mc-2': test_designs,
            'Stratasys450mc-3': test_designs,
            'Stratasys450mc-4': test_designs,
            'Stratasys450mc-5': test_designs,
            'Stratasys450mc-6': test_designs,
        }
        
        selected_indices = []
        for idx, (img_path, label) in enumerate(full_dataset.samples):
            class_name = full_dataset.classes[label]
            if any(f"/{class_name}/{sub}/" in img_path.replace("\\", "/") for sub in desired_test_subfolders.get(class_name, [])):
                selected_indices.append(idx)
                
        max_images_per_class = np.inf
        ...
        custom_test_dataset = Subset(full_dataset, limited_indices)
        custom_test_dataset.classes = full_dataset.classes
        custom_test_dataset.class_to_idx = full_dataset.class_to_idx
        custom_test_dataset.samples = [full_dataset.samples[i] for i in limited_indices]
        custom_test_dataset.imgs = custom_test_dataset.samples
        
        print_subfolder_counts(custom_test_dataset, "TEST")

        image_datasets['train'] = custom_train_dataset
        image_datasets['val'] = custom_test_dataset
        ...
        if (model_name == 'vit_b_16') | (model_name == 'dinov3_vitl16'):
        ...
        print("Rank", rank, "Train Parts", len(custom_train_dataset), "Val Parts", len(custom_test_dataset), "Batch size", batch_size, "Train iters per epoch", len(dataloaders_dict['train']), "Val iters per epoch", len(dataloaders_dict['val']))
...
def run_agent(local_rank, world_size, rank):
    wandb.agent(sweep_id="AIMS/oyy4cmab", count=1, function=lambda: run_model(local_rank, world_size, rank))
...
if __name__ == '__main__':
    ...
    os.environ["NCCL_ALGO"] = "Ring"  # For NVLink topologies
    os.environ["NCCL_CROSS_NIC"] = "0"  # Focus on GPU-direct connections
